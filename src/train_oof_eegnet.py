...
def load_fold(path, device):
    model = EEGNet82(24, 26, input_time_points=801,
                     temporal_kernel_length=25, dropout_rate=0.3)
...
def main():
    ...
    outdir=os.path.join(ROOT,'models','checkpoints','oof_eegnet_k25');os.makedirs(outdir,exist_ok=True)
    ...
    for fold,(train_idx,val_idx) in enumerate(splits):
        ...
        if args.reuse and os.path.exists(path):model=load_fold(path,device)
        else:
            set_seed(args.seed+fold)
            model,_,device=train_deep_learning_model(
                'eegnet',data[train_idx],labels[train_idx],data[val_idx],labels[val_idx],24,801,
                num_epochs=args.epochs,batch_size=64,lr=.005,temporal_kernel=25,
                use_mixup=True,mixup_alpha=.2,noise_std=.07,use_swa=False,
