...
def main():
    ...
    m=EEGNet82(24,26,input_time_points=801,temporal_kernel_length=15,dropout_rate=.3).to(d);centers=nn.Parameter(torch.randn(26,128,device=d)*.02);opt=torch.optim.AdamW(list(m.parameters())+[centers],lr=.003,weight_decay=.03);sched=torch.optim.lr_scheduler.ReduceLROnPlateau(opt,factor=.5,patience=3,min_lr=1e-5);best=(1e9,0);stale=0;path=os.path.join(ROOT,'models','checkpoints','eegnet_center_loss_seed42.pth')
    for e in range(1,90):
        t=time.time();m.train()
        for x,y in tl:
