...
def main():
    ...
    for stage in range(1,5):
        ...
        if stage==4:
            path=os.path.join(ROOT,'models','checkpoints','oof_dcn_0_2000','fold_4_seed_46.pth');m=DeepConvNet(24,26,501,temporal_kernel=15,dropout_rate=.5).to(d);m.load_state_dict(torch.load(path,map_location=d))
        else:
            set_seed(742+stage);m,_,d=train_deep_learning_model('deep_conv_net',x[train],y[train],x[val],y[val],24,501,num_epochs=70,batch_size=64,lr=.005,temporal_kernel=15,use_mixup=False,noise_std=0,early_stopping_patience=15);torch.save(m.state_dict(),path)
